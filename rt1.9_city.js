...
let helpPool ="false" //是否全部助力助力池开关，默认开启
...
!(async () => {
  ...
  inviteCodes = [`-ryUXKVYYWVHZUGWH9PA9JG8D92sziv8`].sort(() => 0.5 - Math.random())
  inCodes = [`-ryUXq8EZWwQZUTFQ4qW8Mc0IcKrKufl`].sort(() => 0.5 - Math.random())
  inviCodes = [`-ryUMex7LgI7NHKEN4qBu-8SP79do-w`].sort(() => 0.5 - Math.random())
  for (let i = 0; i < cookiesArr.length; i++) {
    cookie = cookiesArr[i];
    $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1])
    $.index = i + 1;
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://gitee.com/ANmangxing/mx/raw/master/city.json`, 'timeout': 15000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    ...
    if (readShareCodeRes && readShareCodeRes.code === 200) {
      $.newShareCodes = [...new Set([...inviteCodes,...inCodes,...inviCodes])];
    } else {
      $.newShareCodes = [...new Set([...inviteCodes,...inCodes,...inviCodes])];
    }
